...
export default function PostWritePage() {
  ...
  const submit = async (value: { title: string; content: string }) => {
    if (!localStorage.getItem('accessToken')) {
      await showWarningAlert('로그인이 필요합니다.', '게시글 작성은 로그인 후 이용할 수 있습니다.');
      navigate('/login');
      return;
    }
    ...
    try {
      ...
      await showSuccessAlert('작성 완료', '게시글이 정상적으로 등록되었습니다.');
      navigate(returnPath);
    } catch (error) {
      await showErrorAlert('작성 실패', error instanceof Error ? error.message : '게시글 작성 중 오류가 발생했습니다.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="community-page community-page--narrow">
      ...
      />
      <PostEditorForm mode="create" submitting={submitting} onSubmit={submit} onCancel={() => navigate(returnPath)} />
    </div>
  );
}
