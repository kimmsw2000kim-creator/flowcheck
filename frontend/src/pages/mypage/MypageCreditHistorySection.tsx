...
function MypageCreditHistorySection() {
  ...
  return (
    <section className={styles['mypage-section']}>
      <PageHeader headingLevel={1} eyebrow="CREDITS" title="크레딧 내역" description="충전, 사용, 보상 지급 내역을 확인할 수 있습니다." />
      {loading && <EmptyState title="크레딧 내역을 불러오는 중입니다." description="잠시만 기다려 주세요." />}
      {!loading && errorMessage && <EmptyState title={errorMessage} description="로그인 상태를 확인한 뒤 다시 시도해 주세요." />}
      {!loading && !errorMessage && histories.length === 0 && <EmptyState title="크레딧 내역이 없습니다." description="포인트 충전, 테스트 사용, 커뮤니티 보상 내역이 이곳에 표시됩니다." />}
        ...
        <TableContainer><Table>
          ...
          <tbody>{histories.map((item) => {
            ...
            return <tr key={item.ledgerId}>
              ...
              <td className={positive ? styles['amount-positive'] : styles['amount-negative']}>{positive ? '+' : ''}{item.amount.toLocaleString()}C</td>
            </tr>;
          })}</tbody>
...
export default MypageCreditHistorySection;
